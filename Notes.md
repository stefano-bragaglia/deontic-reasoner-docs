...
| 3-obligation-and-permissibility-queries | 2-weighted-conflict-resolution-regression | story/3-obligation-and-permissibility-queries/2-weighted-conflict-resolution-regression | code |
...
- Feature `2-preference-ordering-and-best-worlds` merged to `main` via epic PR
  [#9](https://github.com/stefano-bragaglia/deontic-reasoner/pull/9) — same auto-merge path as feature 1
  (`mergeable=MERGEABLE`, required `test` CI check passed, merged via `gh pr merge --merge`). Epic file
  marked `DONE`. **Second feature of this iteration fully shipped**: `violates`, `excludes`,
...
Run /pr 3-obligation-and-permissibility-queries/2-weighted-conflict-resolution-regression to open this
story's PR against feature/3-obligation-and-permissibility-queries (all gates green, 100% coverage; no
production code change — test-only story, confirmed against existing queries.py).